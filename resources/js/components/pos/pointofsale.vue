...
<br>
                     <label>Pay</label>
                     <input type="text" class="form-control" v-model="pay" :placeholder="subtotal*vats.vat / 100 + subtotal + '$'"
                     @change="dueResult">
<br>
                     <label>Due</label>
                     <input type="text" class="form-control" v-model="due" required :placeholder="duePay">
<br>
                     <label>Pay By</label>
...
export default {
   ...
   methods: {
      dueResult(event) {
         let num = this.subtotal*this.vats.vat / 100 + this.subtotal;
         let dueResult = num -= event.target.value;
         this.duePay = dueResult.toFixed(1)
      },
      AllProducts() {
         axios
            .get("/api/product")
